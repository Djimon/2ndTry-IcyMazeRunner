...
namespace IcyMazeRunner.Klassen
{
    public class Calculator
    {
        ...
        public static float getDistance(Vector2f VectorOne, Vector2f VectorTwo)
        {
            float distance;
            return distance = (float)Math.Sqrt(Math.Pow(Math.Abs(VectorOne.X-VectorTwo.X),2) + Math.Pow(Math.Abs(VectorOne.Y-VectorTwo.Y),2));
        }

        public static float getWinkel(Vector2f position) //zur x-Achse
        {
            float x = position.X;
            float y = position.Y;
        ...
        public static Vector2f addX (Vector2f vector, float value)
        {
            Vector2f resultVector = new Vector2f(vector.X + value, vector.Y);
            return resultVector;
        }
        public static Vector2f addY(Vector2f vector, float value)
        {
